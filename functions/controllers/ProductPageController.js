...
async function confirmProductRequestSubmit(
  chosenProductId,
  request,
  action,
  is_authenticated
  ...
  if (action === "confirm_availability") {
    // Updates vendor DB entry, adding details of order to confirm. Returns payload.
...
const postProductDetails = async (request, response) => {
  ...
  let payload = await confirmProductRequestSubmit(
    chosenProductId,
    request,
    action,
    is_authenticated
