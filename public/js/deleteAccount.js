        ...
        .then((snapshot) => {
            snapshot.forEach((order) => {
                db.collection("users")
                    ...
                    .then(() => {
                        console.log(
                            `deleted order ${order.data().order_id} from orders_to_confirm`
                        );
                    });
            });
        });
...
function emailWarning() {
    $("#warning-message").text(
        `You're entered email doesn't match the email we have on account. Check the email and try again if you really want to leave. Otherwise just stay!`
    );
}
